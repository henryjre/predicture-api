...
export function showNotificationModal(tradeEvent) {
  return new Promise((resolve) => {
    ...
    if (tradeEvent.ok) {
      ...
      priceLabel.textContent = "Average Price";
      ...
      if (tradeEvent.action === "buy") {
        ...
        receivedValue.textContent =
          tradeEvent.payout + " Shares of " + tradeEvent.choice;
      }
...
function calculatePriceImpactAndAverage(data) {
  const { marketBefore, marketAfter, choice, amountShares, totalCost } = data;
  ...
  const averagePricePerShare =
    amountShares > 0 ? (totalCost / amountShares).toFixed(4) : "N/A";
